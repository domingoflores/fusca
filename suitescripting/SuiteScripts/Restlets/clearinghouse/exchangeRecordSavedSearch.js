...
//					'custrecord_acq_loth_5b_de1_wireverify',
		...
		try {
			...
			var search = nlapiLoadSearch('customrecord_acq_lot', 'customsearch_get_exchange_record_by_hash');
			var filterExp = [].concat(search.getFilterExpression());
			filterExp.push('AND');
			filterExp.push([
				'custrecord_acq_loth_zzz_zzz_identcode', 'is', dataIn.exchangeHash
			]);
			search.setFilterExpression(filterExp);
			
			var exchangeRecordsSearch = search.runSearch();
			exchangeRecords = exchangeRecordsSearch.getResults(0, 1000);
			...
			if( exchangeRecords != null ) {
				...
				for( var i = 0; i < exchangeRecords.length; i++ ) {
					...
					search = nlapiLoadSearch('customrecord_acq_lot_cert_entry', 'customsearch_get_lot_certificates');
					filterExp = [].concat(search.getFilterExpression());
					filterExp.push('AND');
					filterExp.push([
						'custrecord_acq_lotce_zzz_zzz_parentlot', 'is', exchangeRecordId
					]);
					search.setFilterExpression(filterExp);
					
					var lotCertificatesSearch = search.runSearch();
					lotCertificates = lotCertificatesSearch.getResults(0, 1000);
