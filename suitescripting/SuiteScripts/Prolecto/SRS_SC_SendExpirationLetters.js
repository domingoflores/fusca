 ...
 * 
 * Calls the appropriate CRE Profile to send all Pending Expiration Letter Records
 * 
 */
...
const COMMUNICATION_TYPE_IS_EXPIRATION_LETTER = 2; 

function sendExpirationLetters(method) {

	var funcName = "sendExpirationLetters " + method;
...
//		var profileId = context.getSetting("SCRIPT", "custscript_srs_snd_shr_ltrs_cre_profile");
		...
		for (var b = 0; b < batchList.length; b++) {
			...
			if (!failCount) {
				nlapiSubmitField("customrecord_escrow_payment_approvals",rapId,"custrecord_exp_letter_proc_status",SHAREHOLDER_LETTERS_SENT);
				nlapiLogExecution("AUDIT", funcName, "All " + successCount + " pending Expiration Letters for Release Approval Process " + batchList[b].name + " were successfully sent.")				
			} else
				nlapiLogExecution("AUDIT", funcName, "NOT All pending Expiration Letters for Release Approval Process " + batchList[b].name + " were successfully sent.  " + successCount + " were sent; " + failCount + " failed."); 							
		}
...
function findPendingLetters(rapId) {
	...
	try {
		...
		filters.push(new nlobjSearchFilter("custrecord_rc_type", null, "anyof", [COMMUNICATION_TYPE_IS_EXPIRATION_LETTER]));
		...
		nlapiLogExecution("DEBUG", funcName, recList.length + " PENDING EXPIRATION LETTERS found");
...
function findPendingReleaseApprovalRecords() {
	...
	try {
		...
		filters.push(new nlobjSearchFilter("custrecord_exp_letter_proc_status", null, "anyof", SENDING_SHAREHOLDER_LETTERS));
		filters.push(new nlobjSearchFilter("isinactive", null, "is", 'F'));
		columns.push(new nlobjSearchColumn("name"));
		
		var search = nlapiCreateSearch("customrecord_escrow_payment_approvals", filters, columns);
