	...
	this.toJS = function(results, prependHeaders){
		...
		for (var i = 0; i < results.length; i++) {
			...
			for (var j = 0; columns && j < columns.length; j++) {
				...
				if(columns[j].getLabel())
					headers[j]['label'] = columns[j].getLabel();
				
				//getValue
				if (results[i].getValue(columns[j])) 
					o[columns[j].getName()+'_val_'+j] = results[i].getValue(columns[j]);
				//getValue (join)
				if(columns[j].getJoin() && results[i].getValue(columns[j].getName(), columns[j].getJoin()))
					o[columns[j].getName()+'_'+columns[j].getJoin()+'_val_'+j] = results[i].getValue(columns[j].getName(), columns[j].getJoin());
				//getValue (summary)
				if (columns[j].getSummary() && results[i].getValue(columns[j].getName(), null, columns[j].getSummary())) 
					o[columns[j].getName()+'_'+columns[j].getSummary()+'_'+j] = results[i].getValue(columns[j].getName(), null, columns[j].getSummary());
				...
				if (columns[j].getJoin() && columns[j].getSummary() && results[i].getValue(columns[j].getName(), columns[j].getJoin(), columns[j].getSummary())) 
					o[columns[j].getName()+'_'+columns[j].getJoin()+'_summary_'+j] = results[i].getValue(columns[j].getName(), columns[j].getJoin(), columns[j].getSummary());
				//getText
				if(results[i].getText(columns[j].getName()))	
					o[columns[j].getName()+'_txt_'+j] = results[i].getText(columns[j].getName());
				//getText (join)
				if(columns[j].getJoin() && results[i].getText(columns[j].getName(), columns[j].getJoin()))	
					o[columns[j].getName()+'_'+columns[j].getJoin()+'_txt_'+j] = results[i].getText(columns[j].getName(), columns[j].getJoin());
				
				//getText (summary)
				if(columns[j].getSummary() && results[i].getText(columns[j].getName(), null, columns[j].getSummary()))
					if(columns[j].getName().indexOf('formula') === 0)
						o[columns[j].getName()+'_'+columns[j].getSummary()+'_txt_'+j] = results[i].getText(columns[j].getName(), null, columns[j].getSummary());
					else
						o[columns[j].getName()+'_'+columns[j].getSummary()+'_txt'] = results[i].getText(columns[j].getName(), null, columns[j].getSummary());	
					
				//nlapiLogExecution('ERROR','txt', results[i].getText(columns[j].getName()));
			}
			js.push(o);
		}
