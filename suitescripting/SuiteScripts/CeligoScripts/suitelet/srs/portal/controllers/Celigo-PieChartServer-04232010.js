...
SRS.CustomerPortal = (function(){ //private members
	...
	function UiBuilder(){
		...
		this.write = function(request, response){
			//var file = nlapiLoadFile(14007);
			var file = nlapiLoadFile(14009);			
			...
			for (var i = 0; i < currencyIds.length; i++) 
				scriptTags += '<script type="text/javascript" src="https://'+domain+'/app/site/hosting/scriptlet.nl?script=38&deploy=1&compid=772390&jsonp=true&custpage_search_type=customrecord18&custpage_search_id=730&custpage_namespace=CeligoDataCurr' + currencyIds[i] + '&custpage_currencies=' + currencyIds[i] + '"></script>\n';
			
			Util.log('dynamic tags', nlapiEscapeXML(scriptTags));
						
			response.setContentType('HTMLDOC', 'pieChart.html', 'inline');
