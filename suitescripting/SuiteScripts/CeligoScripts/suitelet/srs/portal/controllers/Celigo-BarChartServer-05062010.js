	...
	function UiBuilder(){
		...
		this.write = function(request, response){			
			...
			if(domain && domain != null && domain.indexOf(".sandbox.") > 0)
				...
				domain = "checkout.beta.netsuite.com";
			else
			...
			{
				//scriptTags += '<script type="text/javascript" src="https://checkout.netsuite.com/app/site/hosting/scriptlet.nl?script=38&deploy='+(i + 1)+'&compid=772390&jsonp=true&custpage_search_type=customrecord18&custpage_search_id=708&custpage_namespace=CeligoDataCurr' + currencyIds[i] + '&custpage_currencies=' + currencyIds[i] + '"></script>\n';
				scriptTags += '<script type="text/javascript" src="https://'+domain+'/app/site/hosting/scriptlet.nl?script=38&deploy=1&compid=772390&jsonp=true&custpage_search_type=customrecord18&custpage_search_id=708&custpage_namespace=CeligoDataCurr' + currencyIds[i] + '&custpage_currencies=' + currencyIds[i] + '"></script>\n';
			}
			
			Util.log('dynamic tags', nlapiEscapeXML(scriptTags));
						
			response.setContentType('HTMLDOC', 'barChart.html', 'inline');
    ...
    return { //public members
        main: function(request, response){
            try {
            	...
            	Util.handleError(e, 'Celigo-PieChartServer-04232010.js', ['shiva.alluri@celigo.com'], 6367);
