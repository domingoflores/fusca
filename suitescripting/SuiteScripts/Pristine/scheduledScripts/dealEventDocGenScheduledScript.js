...
define(['N/record', 'N/search', 'N/runtime', 'N/email'],
	 ...
	 * Author: Ken Crossman
	 * NS-1274 - Copy Audience from doc template to document
	 * ___________________________________________________________
	 */
	...
	function(record, search, runtime, email) {
		...
		function createDoc(dealEvent, docGenEvent, exchRecID, sds, exRecs) {
			...
			try {
				...
				if (sds.dtAudience) {
					var audience = sds.dtAudience.split(',');
					docRec.setValue({
						fieldId: 'custrecord_doc_audience',
						value: audience
					});
				}
		...
		function buildStoredDocSelections(dealEvent, docGenEvent) {
			...
			for (var i = 0; i < sdsRecords.length; i++) {
				...
				log.debug('buildStoredDocSelections', 'sds: ' + JSON.stringify(sds));
				sdsList.push(sds);
			}
			return sdsList;
		}
		...
		function buildSDS(sds) {
			return {
				...
				dtAudience: sds.getValue({
					name: 'custrecord_dt_audience',
					join: 'custrecord_ds_doc_template'
				}) || null,
			};
		}
