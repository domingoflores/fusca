 ...
 */
 ...
 /* 
  * DupePI Suitelet
  * v 1.0                   ATP-986             8/12/2019
  * 
  * Robert Bender   rbender@shareholderrep.com
  * 
  * Description:
  * Creates hold records, must be done in this Suitelet because it
  * triggers UE scripts that couldnt have been triggered via the Payment Instruction UE script.
  */

 define(['N/ui/serverWidget', 'N/search', 'N/email', 'N/url', 'N/record', 'N/runtime','/.bundle/132118/PRI_AS_Engine','/SuiteScripts/Pristine/libraries/paymtInstrListLibrary.js', '/SuiteScripts/Pristine/libraries/searchLibrary'], 
 function(serverWidget, search, email, url, record, runtime, appSettings, piListLib, searchLibrary) {
     function onRequest(context) {

         // GET AND POST REQUESTS
         if (context.request.method === 'GET') {

            log.debug('GET','suitelet entered');
            ...
            try{
                ...
                processedPIs = processedPIs.replace( /-/g , ',' );
                if (!Boolean(suitelet_auth) ){ suitelet_auth = null; }
            
                log.debug('DUPE PI SUITELET - start, auth='+suitelet_auth , ', SHid='+SHid+', processedPIs='+processedPIs );

            }catch(e){
                log.error('**** ERROR **** = '+e.message,context.request.headers);
            } finally {
                if (suitelet_auth === "!authorizedSuitelet772390"){

                    // create the hold record from the params
                    var dupesMarked = markShareholderDupes(SHid);

                    if (dupesMarked == "completed"){
                        var suiteletURL = url.resolveRecord({
                            recordType: 'customrecord_paymt_instr', 
                            recordId: current_id,
                            params:	{ 'custscript_shid' : SHid, 'custscript_current_id' : current_id,  'pisprocessed' : processedPIs},      //'custscript_suitelet_auth' : '!authorizedSuitelet772390'
                            isEditMode: false
                        });
                        ...
                        log.debug('DUPE PI SUITELET - END, auth='+suitelet_auth , ', SHid='+SHid+', processedPIs='+processedPIs );
                    ...
                    log.error('**** UNAUTHORIZED SUITELET ATTEMPT *****',context.request.headers);
                }
            }
        ...
        //<ATP-986>	************************************************************************************
        /**
         * This places dupes on hold just like ATP-856, only for 1 Shareholder. 
         * @param {integer} SHid 
         */
		function markShareholderDupes(SHid){
			...
			try{
				...
				var PISBsavedSearch = search.create({
					type: "customrecord_paymt_instr_submission",
					filters:
					[
					   ["custrecord_pisb_shareholder","anyof",SHid], 
					   "AND", 
					   ["isinactive","is","F"], 
					   "AND", 
					   ["custrecord_pisb_submission_status","noneof", subSts.Promoted , subSts.Canceled], 
					   "AND", 
					   ["count(id)","greaterthan","1"]
					],
					columns:
					[
					   ...
					   search.createColumn({
						  name: "custrecord_pisb_paymt_instr_type",
						  summary: "GROUP"
					   }),
					   search.createColumn({
						  name: "custrecord_pisb_shareholder",
						  summary: "GROUP"
					   }),
					   search.createColumn({
						  name: "custrecord_pisb_deal",
						  summary: "GROUP"
					   }),
					   search.createColumn({
						  name: "custrecord_pisb_exchange",
						  summary: "COUNT"
					   }),
					   search.createColumn({
						  name: "formulatext",
					   ...
					   search.createColumn({
						  name: "formulatext",
						  summary: "MIN",
						  formula: "REPLACE(NS_CONCAT(TO_CHAR({custrecord_pisb_paymethod})), ',', ',')"
					   })
					]
				 });
				 var searchResultCount = PISBsavedSearch.runPaged().count;
				 log.debug("customrecord_paymt_instr_submissionSearchObj result count",searchResultCount);
				 PISBsavedSearch.run().each(function(result){
					// .run().each has a limit of 4,000 results
					return true;
				 });
                        ...
                        if (PIdupes[i].custrecord_pi_paymt_instr_type == piType.ExchangeRecord || PIdupes[i].custrecord_pi_paymt_instr_type == piType.AcquiomDeal && PIdupes[i].ON_DUPE_HOLD[x] != PIdupes[i].internal_ids[x] ){   //@TODO eNum 10=piType.AcquiomDeal  11=piType.ExchangeRecord
                            if ( PIdupes[i].ExRecOrDealDupe[x] == true ){
                                if (createHoldRecords == true && !Boolean(PIdupes[i].ON_DUPE_HOLD[x])  ){
                                    ...
                                    log.debug('About to SAVE HOLD REC, holdRecord=', JSON.stringify(holdRecord) );
                                    holdRecord.save();
                                    
                                    // flag the PI as updated
                                    record.submitFields({ 
                                        type: 'customrecord_paymt_instr', 
                                        id: PIdupes[i].internal_ids[x],
                                        values: {
                                            'custrecord_pi_updating': true
                                        },
                                        options: {
                                            enableSourcing: false, 
                                            ignoreMandatoryFields : true
            ...
            for (var x=0; x<allData[i].length; x++){ //loop thru each PI & PISB of the shareholder
                ...
                if (allData[i][x].type == "PI" ){
                    ...
                    for (var y=0; y<PIinternalids.length; y++){
                        processedPIs.push(allData[i][x].internal_ids[y]);
        ...
        if (processedPIs == []){ processedPIs = ['NONE']}
        log.debug('PIs that were processed=',processedPIs);

        return "completed";

        } // end markShareholderDupes function
