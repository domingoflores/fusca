...
function scheduleCopyTemplate(){
	...
	templateProjectTasksColumns[12] = new nlobjSearchColumn('custevent_proj_task_managing_dept', null, null);
	templateProjectTasksColumns[13] = new nlobjSearchColumn('custevent_proj_task_docket_entry', null, null);
	templateProjectTasksColumns[14] = new nlobjSearchColumn('custevent_proj_task_entity_status', null, null);
...
//
// completeTask: A workflow action to the more generic completeTaskById
//
function completeTask() {
	//Get passed parameters
	var context = nlapiGetContext();
	completeTaskById(context.getSetting('SCRIPT','custscript_ct_trg_proj_tsk_id'))
}
...
//
//cleanUpActiveFlags:  A scheduled script frontend to the resolveActiveFlags library function.
//
function cleanUpActiveFlags() {
	nlapiLogExecution('DEBUG', 'SRSProjects.cleanUpActiveFlags', 'Starting cleanUpActiveFlags...');
	resolveActiveFlags();
	nlapiLogExecution('DEBUG', 'SRSProjects.cleanUpActiveFlags', 'Ending cleanUpActiveFlags.');

}

//
//updateTasksOnOppStatusChange:  A workflow action that takes a project internal id, an array opp status ids and an
//							     action type (complete, na, inprogress) and updates the tasks in the deal project
//								 that are marked with the Opp. Status Updates flag.
//
function updateTasksOnOppStatusChange() 
{
	nlapiLogExecution('DEBUG', 'SRSProjects.updateTasksOnOppStatusChange', 'Starting updateTasksOnOppStatusChange...');

		var context = nlapiGetContext();
		
		var projectId = context.getSetting('SCRIPT', 'custscript_proj_opp_sub_proj_id');
		var statusId = context.getSetting('SCRIPT', 'custscript_proj_opp_sub_status').split(',');//since a parameter apparently can't be multiselect, do comma separated internal ids instead.
		var action = context.getSetting('SCRIPT', 'custscript_proj_opp_sub_action');
		
		//Check to see if there is a projectid, exit if not
		nlapiLogExecution('DEBUG', 'SRSProjects.updateTasksOnOppStatusChange', 'Checking for ProjectID.');
		if (projectId != null && projectId != '')	
		{
			nlapiLogExecution('DEBUG', 'SRSProjects.updateTasksOnOppStatusChange', 'ProjectID exists:' + projectId);
			//first get all the task records for the project that are subscribed to the opportunity.
			var filters = new Array(); 
			var columns = new Array();
			var result = null;
			
			filters[0] = new nlobjSearchFilter('custevent_proj_task_entity_status',null,'anyOf',statusId);
			filters[1] = new nlobjSearchFilter('company',null,'anyOf',projectId);
			filters[2] = new nlobjSearchFilter('status',null,'noneOf','COMPLETE');
			filters[3] = new nlobjSearchFilter('custentity_proj_is_template','job','is','F');
			...
			if(result != null) {
				for(var os1 = 0; os1 < result.length;os1++) {
					switch(action.toLowerCase()) {
					case 'complete':
						nlapiLogExecution('DEBUG', 'SRSProjects.updateTasksOnOppStatusChange', 'Completing task: '+result[os1].getId());
						completeTaskById(result[os1].getId());
						break;
					case 'na':
						nlapiLogExecution('DEBUG', 'SRSProjects.updateTasksOnOppStatusChange', 'NA Action (none)');
						break;
					case 'inprogress':
						nlapiLogExecution('DEBUG', 'SRSProjects.updateTasksOnOppStatusChange', 'In Progress Action (none)');
						break;
					default:
						nlapiLogExecution('DEBUG', 'SRSProjects.updateTasksOnOppStatusChange', 'Bad parameter.  Must be: complete,inprogress, or na');
					}
				}
			} else {
				nlapiLogExecution('DEBUG', 'SRSProjects.updateTasksOnOppStatusChange', 'Found no subscribed records.');
			}
			
			nlapiLogExecution('DEBUG', 'SRSProjects.updateTasksOnOppStatusChange', 'Ending updateTasksOnOppStatusChange.');
			}
		else
			{
			nlapiLogExecution('DEBUG', 'SRSProjects.updateTasksOnOppStatusChange', 'Project does not exist, nothing to update.');
			}
	}
...
//
//updateTasksOnDocketStageChange:  A workflow action that takes a project internal id, an array Docket Stage ids and an
//							     action type (complete, na, inprogress) and updates the tasks in the deal project
//								 that are marked with the Docket Stage subscription.
//
function updateTasksOnDocketStageChange() {
	nlapiLogExecution('DEBUG', 'SRSProjects.updateTasksOnDocketStageChange', 'Starting updateTasksOnDocketStageChange...');
	
	var context = nlapiGetContext();
	
	var projectId = context.getSetting('SCRIPT', 'custscript_proj_deal_sub_proj_id');
	var statusId = context.getSetting('SCRIPT', 'custscript_proj_deal_sub_status').split(',');//since a parameter apparently can't be multiselect, do comma separated internal ids instead.
	var action = context.getSetting('SCRIPT', 'custscript_proj_deal_sub_action');
	...
	filters[0] = new nlobjSearchFilter('custevent_proj_task_docket_entry',null,'anyOf',statusId);
	filters[1] = new nlobjSearchFilter('company',null,'anyOf',projectId);
	filters[2] = new nlobjSearchFilter('status',null,'noneOf','COMPLETE');
	filters[3] = new nlobjSearchFilter('custentity_proj_is_template','job','is','F');
	...
	if(result != null) {
		for(var os1 = 0; os1 < result.length;os1++) {
			switch(action.toLowerCase()) {
			case 'complete':
				nlapiLogExecution('DEBUG', 'SRSProjects.updateTasksOnDocketStageChange', 'Completing task: '+result[os1].getId());
				completeTaskById(result[os1].getId());
				break;
			case 'na':
				nlapiLogExecution('DEBUG', 'SRSProjects.updateTasksOnDocketStageChange', 'NA Action (none)');
				break;
			case 'inprogress':
				nlapiLogExecution('DEBUG', 'SRSProjects.updateTasksOnDocketStageChange', 'In Progress Action (none)');
				break;
			default:
				nlapiLogExecution('DEBUG', 'SRSProjects.updateTasksOnDocketStageChange', 'Bad parameter.  Must be: complete,inprogress, or na');
			}
		}
	} else {
		nlapiLogExecution('DEBUG', 'SRSProjects.updateTasksOnDocketStageChange', 'Found no subscribed records.');
	}
	
	nlapiLogExecution('DEBUG', 'SRSProjects.updateTasksOnDocketStageChange', 'Ending updateTasksOnDocketStageChange.');
}

//
//cancelProjectTaskScheduled:  A scheduled script to call the cancelProjectTasks library function.  In the event of a
//							   lost deal, there is a need to cancel remaining tasks.
//
function cancelProjectTaskScheduled() {
	nlapiLogExecution('DEBUG', 'SRSProjects.cancelProjectTaskScheduled', 'Starting cancelProjectTaskScheduled...');
	var context = nlapiGetContext();
	var projectId = context.getSetting('SCRIPT', 'custscript_proj_cancel_deal_schd_pid');
	
	cancelProjectTasks(projectId);
	
	nlapiLogExecution('DEBUG', 'SRSProjects.cancelProjectTaskScheduled', 'Ending cancelProjectTaskScheduled.');
}

//
//cancelProjectTasksWorkflow:  A workflow action to call the cancelProjectTaskScheduled scheduled script function.  In the event of a
//							  lost deal, there is a need to cancel remaining tasks.  This workflow action keys into the opp status
//							  for that purpose.
//
function cancelProjectTasksWorkflow() {
	nlapiLogExecution('DEBUG', 'SRSProjects.cancelProjectTasksWorkflow', 'Starting cancelProjectTasksWorkflow...');
	var context = nlapiGetContext();
	var params = new Object();
	if(context.getSetting('SCRIPT', 'custscript_proj_cancel_deal_proj_id') != null && context.getSetting('SCRIPT', 'custscript_proj_cancel_deal_proj_id') != '') {
		params['custscript_proj_cancel_deal_schd_pid'] = context.getSetting('SCRIPT', 'custscript_proj_cancel_deal_proj_id'); //The project id that has the tasks that should be cancelled.
	    
	    var status = nlapiScheduleScript('customscript_proj_cancel_sched', 'customdeploy_proj_cancel_sched', params);
	    if (status == 'QUEUED')
	    	nlapiLogExecution('DEBUG', 'SRSProjects.cancelProjectTasksWorkflow', 'Ending cancelProjectTasksWorkflow.');
	        return 0;
	} else {
		nlapiLogExecution('DEBUG', 'SRSProjects.cancelProjectTasksWorkflow', 'Ending cancelProjectTasksWorkflow... no project to cancel! Value:'+context.getSetting('SCRIPT', 'custscript_proj_cancel_deal_proj_id'));
	}
}

//
//simpleCopyTaskWorkflow: Workflow front end to the simple script copying.
//
//
function simpleCopyTaskWorkflow() {
	nlapiLogExecution('DEBUG', 'SRSProjects.simpleCopyTaskWorkflow', 'Starting simpleCopyTaskWorkflow...');
	var context = nlapiGetContext();
	if(context.getSetting('SCRIPT','custscript_simplecopy_sid') == null) {
		nlapiLogExecution('ERROR', 'SRSProjects.simpleCopyTaskWorkflow', 'The source task id was null, cannot run workflow action');
		return null;
	}
	var tempTask = nlapiLoadRecord('projecttask',context.getSetting('SCRIPT','custscript_simplecopy_sid'));
	simpleCopyTask(tempTask.getId(),tempTask.getFieldValue('company'),tempTask.getFieldValue('parent'),true);
	nlapiLogExecution('DEBUG', 'SRSProjects.simpleCopyTaskWorkflow', 'Ending simpleCopyTaskWorkflow.');
}


//
//setProjectName: Workflow front end for changing project names, usually from customer related workflows.
//
//
function setProjectNameWorkflow() {
	nlapiLogExecution('DEBUG', 'SRSProjects.setProjectNameWorkflow', 'Starting setProjectNameWorkflow...');
	var context = nlapiGetContext();
	if(context.getSetting('SCRIPT','custscript_setprojectname_pid') == null || context.getSetting('SCRIPT','custscript_setprojectname_name') == null) {
		nlapiLogExecution('ERROR', 'SRSProjects.setProjectNameWorkflow', 'The project id or the new name (or both) was null.');
		return null;
	}
	setProjectName(context.getSetting('SCRIPT','custscript_setprojectname_pid'),context.getSetting('SCRIPT','custscript_setprojectname_name'));
}
