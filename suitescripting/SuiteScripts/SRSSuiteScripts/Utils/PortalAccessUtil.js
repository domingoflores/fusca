...
if (nlapiGetContext().getEnvironment() == 'PRODUCTION' ) {
    IOSUITE_API_BASE_URL = 'https://iosuite.srsacquiom.com/api/v1/';
    PORTAL_BASE_URL = 'https://www.srscomport.com/';
    PORTAL_ENV = 'PRODUCTION';

}
else {
    IOSUITE_API_BASE_URL = 'https://dev.iosuite.srsacquiom.com/api/v1/';
...
function getShareholderDataAccess(contact) {
    ...
    if (results.length > 1) {
        throw 'MULTIPLE_REPORT_RECORDS_FOUND';
    }
...
function getOrCreateShareholderDataAccess(contact, createRecord) {
    ...
    if (results == null && createRecord) {
        ...
        /*
         * Since an SDA didn't exist, we need to add the UUID to the contact record
         * TJ Tyrrell - September 28, 2015
         */
        var iosuiteApi = IOSUITE_API_BASE_URL + 'uuid',
            uuidContents = nlapiRequestURL(iosuiteApi),
            uuid = uuidContents.getBody();

        nlapiLogExecution('DEBUG', 'PortalAccessUtil.getOrCreateShareholderDataAccess uuid URL', iosuiteApi );
        nlapiLogExecution('DEBUG', 'PortalAccessUtil.getOrCreateShareholderDataAccess uuid', uuid );

        var nsContact = nlapiLoadRecord('contact', contact.internalid);
    ...
    // make sure the top level parent element is set properly
    if (record.getFieldValue('custrecord_toplevelparent') == null) {
        record.setFieldValue('custrecord_toplevelparent', contact.toplevelParent);
        nlapiSubmitRecord(record);
        return getShareholderDataAccess(contact);
    }
...
function getRootContact(email) {
    ...
    if (searchResults.length > 1) {
        throw 'DUPLICATE_CONTACTS_FOUND';
    }

    var result = searchResults[0];

    var isHasChangedPassword = hasChangedPassword(email);
    if (isHasChangedPassword == null) {
        isHasChangedPassword = false;
    }
...
function getContacts(parentId) {
    ...
    if (parentId == null || parentId.lenth == 0) {
        return null;
    }
...
function render(request,response) {
    ...
    try {
        var contact = loadContact(email.trim(), parent);
        var content = '';
        if (!contact.toplevelParent || contact.toplevelParent == null) {
        ...
        if (error == 'NO_CONTACT_FOUND') {
            ...
            var duplicateUrl = nlapiResolveURL('TASKLINK', 'LIST_ENTITY_DUPLICATES') + '&type=contact&id=' + firstContact.internalid;
...
function renderMultiParent(contact) {
    ...
    // loop through each parent and display
    for (var i = 0; i < contact.allTopLevelParents.length; i++) {
...
function renderContact(contact) {
    ...
    var tophtml = '<html>\n<head>\n<LINK REL=StyleSheet HREF="/core/media/media.nl?id=16339&c=772390&h=8cf1ac9e2791667ea641&_xt=.css"/>\n</head>\n<body>';
    ...
    if (results == null || results.length == 0) {
        return tophtml.replace(/ONLINE_REG_RECORDS/g, '') + html;
    }
...
function sendRequestForCorrectSecurityAnswersEmail(regId, FILE_ID) {
    ...
    var subj = 'Action Required: SRS ComPort Registration Confirmation';
...
function renderDuplicate(request) {
    ...
    var duplicateUrl = nlapiResolveURL('TASKLINK', 'LIST_ENTITY_DUPLICATES') + '&type=contact&id=';
...
function renderPendingList(request,statusList) {
    ...
    var results = nlapiSearchRecord('customrecord13', null, filters, columns);
    if (results == null || results.length == 0) {
        return 'No Pending Registrations Found';
    }
    ...
    for (var i = 0; i < results.length; i++) {
        ...
        if (hash.shareholder == null || hash.shareholder.length == 0) {
            hash.shareholder = result.getValue('custrecord58');
        }

        hash.deal = result.getText('custrecord18');
        if (hash.deal == null || hash.deal.length == 0) {
            hash.deal = result.getValue('custrecord_temp_deal_name');
        }
...
function approveRegistration(request) {
    ...
    if (regId == null || regId.length == 0) {
        return 'No Registration ID Found in Request';
    }
    var rcd = nlapiLoadRecord('customrecord13',regId);
    ...
    /*
     * Since this function is never used, TJ Tyrrell changed the status id from 2 (approved) to 3 (complete) to improve the workflow
     * If there is ever an issue, just change 3 back to 2
     * 5/3/2016
     */
...
function renderPendingRegistration(request) {
    ...
    if (nextRegId == null) {
        nextRegId = getNextRegistrationId(0);    // get the first one
    }
    ...
    if (deal == null || deal.length == 0) {
        ...
        if (deal == null || deal.length == 0) {
            ...
            deal += '<a href="' + portletUrl + '&page=matchDealRegistration&registrationid=' + regId + '">(find match)</a>';
        }
    }
    else {
    ...
    if (shareholder == null || shareholder.length == 0) {
        ...
        if (shareholder == null || shareholder.length == 0) {
            ...
            shareholder += ' <a href="' + portletUrl + '&page=matchShareholderRegistration&registrationid=' + regId + '">(find match)</a>';
        }
    }
    else {
    ...
    docFilters.push(new nlobjSearchFilter('custrecord_escrow_customer', null, 'anyOf', dealId));
...
function matchShareholderRegistration(request) {
    ...
    var shareholder = rcd.getFieldText('custrecord17');
    if (shareholder != null && shareholder.length > 0) {
        return 'Match for shareholder already made.';
    }
    shareholder = rcd.getFieldValue('custrecord58');
...
function copyOlrData(request) {
    ...
    for (var i = 0; i < contacts.length; i++) {
        ...
        if (copyToContactEmail == null && copyToContactFirstName == null && copyToContactLastName == null) {
            continue;
        }

        var conRcd = nlapiLoadRecord('contact', contact.internalid);

        if (copyToContactEmail != null) {
            conRcd.setFieldValue('email', copyToContactEmail);
        }
        if (copyToContactFirstName != null) {
            conRcd.setFieldValue('firstname', copyToContactFirstName);
        }
        if (copyToContactLastName != null) {
            conRcd.setFieldValue('lastname', copyToContactLastName);
        }
...
function createNewContactFromRegistration(request) {
    ...
    if (shareholderId == null) {
        return;    // can't do anything here until a shareholder record exists
    }
    if (dealId == null) {
        return;            // need dealId to attach record to
    }
    ...
    if (olrEmail == null && olrFirstName == null && olrLastName == null) {
        return;
    }
    ...
    if (olrFirstName != null) {
        contact.setFieldValue('firstname', olrFirstName);
    }
    if (olrLastName != null) {
        contact.setFieldValue('lastname', olrLastName);
    }
...
function copyShareholderRegistration(request) {
    ...
    if (olrEmail != null && olrEmail.length > 0) {
        html += '<td><input type="checkbox" name="copyToShareholderEmail" value="' + olrEmail + '"/></td>';
    }
    else {
        html += '<td></td>';
    }
    ...
    for (var i = 0; contacts != null && i < contacts.length; i++) {
        var contact = contacts[i];
        if (olrEmail != null && olrEmail.length > 0) {
            html += '<td><input type="checkbox" name="copyToContactEmail' + i + '" value="' + olrEmail + '"/></td>';
        }
        else {
            html += '<td></td>';
        }
...
function matchDealRegistration(request) {
    ...
    var deal = rcd.getFieldText('custrecord18');
    if (deal != null && deal.length > 0) {
        return 'Match for deal already made.';
    }
    deal = rcd.getFieldValue('custrecord_temp_deal_name');
...
function postShareholderMatch(request) {
    ...
    if (shareholderId == null || shareholderId.length == 0) {
        return 'No Shareholder ID Found in Request';
    }
...
function postDealMatch(request) {
    ...
    if (dealId == null || dealId.length == 0) {
        return 'No Deal ID Found in Request';
    }
...
function formTableLineLink(fullText, alignment, theUrl, target) {
    if (theUrl == null) {
        return formTableLine(fullText,alignment);
    }
    if (fullText == null) {
        fullText = '';
    }
    ...
    if (fullText.length > 70) {
        displayValue = displayValue.substring(0,70) + '...';
    }
...
function addCompanyAccess(request) {
    ...
    if (type == 'deal') {
        ...
        // if it is non-null, simply add the new shareholder
        ...
        // if it is null, remove all shareholders and set the investor group
        else {
            //var parents = getTopLevelParents(contact);
...
function givethPortalAccess(contact) {
    ...
    var iosuiteApi = IOSUITE_API_BASE_URL + 'uuid',
        uuidContents = nlapiResolveURL( iosuiteApi ),
        uuid = uuidContents.getBody();
    ...
    for (var i = 1; i <= nlapiGetLineItemCount('contactroles'); i++) {
        if (nlapiGetLineItemValue('contactroles', 'contact',i) == contact) {
            ...
            givethPortalAccessLine(contact,i, uuid);
            return;
        }
            ...
            givethPortalAccessLine(nlapiGetLineItemValue('contactroles', 'contact', i), i, uuid);
        }
    }
...
function givethPortalAccessLine(contact, rowNumber, uuid) {
    ...
    nlapiSetLineItemValue('contactroles', 'passwordalt', rowNumber, user + pwd);
...
function renderEmailReportSync(contact) {
    ...
    try {
        ...
        if (!compareLists(reportShareholders, internalIdToArray(contact.viewableShareholders))) {    // in this case we want to remove all of the shareholders and just set an investor group
        ...
        if (error == 'MULTIPLE_REPORT_RECORDS_FOUND') {
            return createLinkAlt(suitletUrl, 'forceSync', 'No (Force Sync)', contact, null, null);
        }
        else {
            throw error;
...
/**
 * Force synchronization between portal access and shareholder data access
 * @param  {Object} request HTTP Request Object
 * @return {null}         void
 */
function doForceSync(request) {
    ...
    // Trigger portal support case creation
    createPortalSupportCase(contact.internalid);
    return null;
}

/**
 * Generate a new NetSuite CASE to trigger the workflow
 * that sends out emails to the contact to create a password in Portal
 * @param  {NetSuite Record} contactId The contact record id for the user requesting Portal access
 * @return {null} void
 */
function createPortalSupportCase(contactId) {

    var contact = nlapiLoadRecord('contact', contactId);
    var email = contact.getFieldValue('email');
    var company = contact.getFieldValue('company');
    var portalUUID = contact.getFieldValue('custentity_portal_hash');
