 ...
 *****************/
// 
// beforeLoad:  The entry point for managing the project task changes.
//
function beforeLoad(opType) {
	var opType = opType.toString();
	nlapiLogExecution('DEBUG','SRSProjectTask.beforeLoad','Type: '+opType+'; Record Type: '+nlapiGetRecordType().toLowerCase()+'; Record ID: '+nlapiGetRecordId());
	switch(opType) {
		case 'create':
			nlapiLogExecution('DEBUG','SRSProjectTask.beforeLoad','Trying to create now!!!');
			break;
		case 'edit':
			nlapiLogExecution('DEBUG','SRSProjectTask.beforeLoad','Trying to edit now!!!');
			break;
		case 'xedit':
			nlapiLogExecution('DEBUG','SRSProjectTask.beforeLoad','Running xedit routine...');
			break;
		case 'delete':
			nlapiLogExecution('DEBUG','SRSProjectTask.beforeLoad','Running delete routine...');
			break;
		default:
			nlapiLogExecution('DEBUG','SRSProjectTask.beforeLoad','Something is wrong because I have defaulted.  Type: '+opType);
	...
	if(nlapiGetRecordId() != -1 && nlapiGetRecordType().toLowerCase() == 'projecttask') {
		switch (opType) {
			...
			case 'xedit':
				...
				if(oldRec.getFieldValue('status') != newRec.getFieldValue('status') && newRec.getFieldValue('status') == 'COMPLETE') {
...
// 
// afterSubmit:  The entry point for managing the project task changes.
//
function afterSubmit(opType) {
	...
	if(nlapiGetRecordId() != -1 && nlapiGetRecordType().toLowerCase() == 'projecttask') {
		switch (opType) {
			case 'create':
				nlapiLogExecution('DEBUG','SRSProjectTask.afterSubmit','Trying to create now!!!');
				break;
			case 'edit':
				nlapiLogExecution('DEBUG','SRSProjectTask.afterSubmit','Trying to edit now!!!');
				break;
			case 'xedit':
				nlapiLogExecution('DEBUG','SRSProjectTask.afterSubmit','Trying to xedit now!!!');
				break;
			case 'delete':
				nlapiLogExecution('DEBUG','SRSProjectTask.afterSubmit','Running delete routine...');
				break;
			default:
				nlapiLogExecution('DEBUG','SRSProjectTask.afterSubmit','Something is wrong because I have defaulted.  Type: '+opType);
		}
	} else {
		nlapiLogExecution('DEBUG','SRSProjectTask.afterSubmit','Bypassed everything...');
	}
}
