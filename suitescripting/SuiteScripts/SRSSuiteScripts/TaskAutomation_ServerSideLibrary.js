...
/// Scott Streule - 4/17/18 - Updated hard coded URL to use nlapiResolveURL() - LINE #488
...
{
	generateTasksByEvent(eventId, transId, entityId, taskFormId, "6367", null);	// 6367 == Dan Urbano
}

//function to generate tasks
function generateTasksByEvent(eventId, transId, entityId, taskFormId, userId, caseId)
{
    ...
    for (var t in parentTasks)
	{
        ...
        var taskInfo = createTask(taskConfigId, userId, entityId, transId, null, taskFormId, caseId);   // taskConfigRecordID, assignee, oppEntity, oppTranid, parentTaskId, formId
        ...
        for (var c in childTasks)                                                               // iterate through and generate tasks
        {
            ...
            var childTaskInfo = createTask(childTaskConfigId, "6367", entityId, transId, taskInfo.taskId, taskFormId, caseId)
...
{
    return createTask(taskConfigRecordID, assignee, oppEntity, oppTranid, parentTaskId, null, null);
}

//function to create task
function createTask(taskConfigRecordID, assignee, oppEntity, oppTranid, parentTaskId, formId, caseId)
{
    ...
    if (taskConfigRecord != null)
    {
		...
		if(oppTranid != null && oppTranid != "")
        	newTask.setFieldValue("transaction", oppTranid);
		if(caseId != null && caseId != "")
        	newTask.setFieldValue("supportcase", caseId);
		
        newTask.setFieldValue("custevent30", taskConfigRecord.getFieldValue("custrecord_notes"));
        newTask.setFieldValue("custevent_display_order",taskConfigRecord.getFieldValue("custrecord_order"));
...
function addTaskSublistTab(tabName, txId, txRecordType)
{
	...
	columns[4] = new nlobjSearchColumn('custevent_display_order');
    ...
    for(var i=0 ; searchResults != null && i < searchResults.length; i++)                                       // setup view url
    {
        ...
        //var viewUrl = "https://system.na2.netsuite.com" + nlapiResolveURL ('RECORD', result.getRecordType(), id, false);    // display the view link
...
function countOpenSubtasks(parentTaskId, txId, txRecordType)
{
    ...
    subFilters[0] = new nlobjSearchFilter('status', null, 'noneof',  'COMPLETE');
...
{ 
    var x = a.getValue('custevent_display_order') 
    var y = b.getValue('custevent_display_order') 
