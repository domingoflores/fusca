...
if ( request.getMethod() == 'GET' )
{
    ...
    if (lockit == 0 || lockit == null)
    {
        ...
        var distype_comp = 'disabled';
        var distype_ops = 'disabled';
    }
    else
    {
        var distype = 'entry';
        var distype_comp = 'entry';
        var distype_ops = 'entry';
    }

    //Set the current dashboard view
    var currview = request.getParameter('currview');
    //if (currview == 'Comp') {distype_ops = 'hidden'} else  //Blue  
    //if (currview == 'Ops') {distype_comp = 'hidden'} else  //Green
    currview = 'All';
    //
    ...
    var runmode = request.getParameter('mode'); //See if this for Cases with expiration dates or just blanks
    ...
    else
    {
        ...
        currdate.setDate(currdate.getDate() - 900);
        var searchday = nlapiDateToString(currdate)

        var filter_date = new Array();

        if (runmode == '1')
        {
            filter_date[0] = new nlobjSearchFilter('custevent_release_expire_date', null, 'isempty');  //Just blanks
        }
        else
        {
            filter_date[0] = new nlobjSearchFilter('custevent_release_expire_date', null, 'onorafter', searchday); //Force to have a date 
        }

        caseResults = nlapiSearchRecord('supportcase', 'customsearch1039', filter_date);
...
//    var form = nlapiCreateForm('Open Releases Dashboard');  //Setup the selection form
    ...
    //form.addField('custpage_datefilter','date','Date On or After')
    //form.addField('custpage_currentview','radio','All', 'All')
    //form.addField('custpage_currentview','radio','Compliance View', 'Comp')
    //form.addField('custpage_currentview','radio','Operation View', 'Ops')
    //form.getField('custpage_currentview').setDefaultValue(currview)
...
//    sublist.addField('custpage_lockedind','text',"<a href='#' style=color:red title='Who'>"+"Who"+"</a>");
    ...
    sublist.addField('custpage_assignedto','textarea','Assigned To');
//    sublist.addField('custpage_assignedtocomp','text','Compliance Expert');
...
//    sublist.addField('custpage_nextduedate','textarea','Expire');
    ...
    sublist.addField('custpage_schedunsched','text','Scheduled');
    sublist.addField('custpage_casequeue','text','Type');
    ...
    var ee1  = sublist.addField('custpage_releaseexpensefundsdue','select','<a href="#" style="color:#339933" title="Escrow Exchange (EE)">'+'EE'+'</a>').setDisplayType(distype_ops);
    var exp  = sublist.addField('custpage_release','select','<a href="#" style="color:#339933" title="Expense Fund Location and Being Released (EXP$)">'+'EXP$'+'</a>').setDisplayType(distype_ops);
//    var sch  = sublist.addField('custpage_releasescheduled','select',"<a href='#' style=color:#339933 title='Release Schedule (SCH)'>"+"SCH"+"</a>").setDisplayType(distype_ops);
//    var pm   = sublist.addField('custpage_releasepayoutmechanics','select',"<a href='#' style=color:#339933 title='Release Payout Mechanics (PM)'>"+"PM"+"</a>").setDisplayType(distype_ops);
//    var v1   = sublist.addField('custpage_releasemoveverified','select',"<a href='#' style=color:#339933 title='Funds / Shares Distributed (VER)'>"+"VER"+"</a>").setDisplayType(distype_ops);
//    var all  = sublist.addField('custpage_releaseallocated','select',"<a href='#' style=color:#339933 title='Booked, Allocated, Released, Negative Shareholder? (ALL)'>"+"ALL"+"</a>").setDisplayType(distype_ops);
    
//    var ql   = sublist.addField('custpage_releasequietletter','select',"<a href='#' style=color:#3399FF style=color:#3399FF title='Release Quiet Letter (QL)'>"+"QL"+"</a>").setDisplayType(distype_comp);
//    var el   = sublist.addField('custpage_releaseexpirationletter','select',"<a href='#' style=color:#3399FF title='Release Expiration Letter (EL)'>"+"EL"+"</a>").setDisplayType(distype_comp);
//    var il   = sublist.addField('custpage_releaseinstructionletter','select',"<a href='#' style=color:#3399FF title='Release Instruction Letter (IL)'>"+"IL"+"</a>").setDisplayType(distype_comp);
//    var sl1  = sublist.addField('custpage_releaseshareholderletter','select',"<a href='#' style=color:#3399FF title='Release Shareholder Letter (SL)'>"+"SL"+"</a>").setDisplayType(distype_comp);
//    var lgl  = sublist.addField('custpage_releaselegalapproval','select',"<a href='#' style=color:#3399FF title='Legal Approval (LA)'>"+"LA"+"</a>").setDisplayType(distype_comp);
//    var rp   = sublist.addField('custpage_releasetoportal','select',"<a href='#' style=color:#3399FF title='Release to Portal (RP)'>"+"RP"+"</a>").setDisplayType(distype_comp);
//    var esn  = sublist.addField('custpage_releaseescrowstatementnews','select',"<a href='#' style=color:#3399FF title='Release Escrow Statement News (ESN)'>"+"ESN"+"</a>").setDisplayType(distype_comp);
...
//    ql.addSelectOption('1','D - (QL)'); ql.addSelectOption('3','S - (QL)'); ql.addSelectOption('2', 'N/A - (QL)'); ql.addSelectOption('4', '* - (QL)'); ql.addSelectOption('', '- - (QL)');
//    el.addSelectOption('1','D - (EL)'); el.addSelectOption('3','S - (EL)'); el.addSelectOption('2', 'N/A - (EL)'); el.addSelectOption('4', '* - (EL)'); el.addSelectOption('', '- - (EL)');
//    il.addSelectOption('1','Auto - (IL)'); il.addSelectOption('2','D - (IL)'); il.addSelectOption('3', 'A - (IL)'); il.addSelectOption('4', 'S - (IL)'); il.addSelectOption('5', 'N/A - (IL)'); 
//    il.addSelectOption('6', '* - (IL)'); il.addSelectOption('', '- - (IL)');
//    sl1.addSelectOption('1','D - (SL)'); sl1.addSelectOption('3','S - (SL)'); sl1.addSelectOption('2', 'N/A - (SL)'); sl1.addSelectOption('4', '* - (SL)'); sl1.addSelectOption('', '- - (SL)');
...
//            sublist.setLineItemValue('custpage_assignedtocomp', s+1, '<a href="#" title="Click here to edit the case." onclick="Popup=window.open(\''+nlapiResolveURL('RECORD','supportcase',currCase.getId(),'VIEW')+'&l=T&e=T'+'\',\'Popup\',\'toolbar=no, location=yes,status=no,menubar=no,scrollbars=yes,resizable=yes, width=1200,height=600,left=50,top=50\'); return false;">'+currCase.getText('custevent_assignedto_comp_exp')+'</a>');
            //Release Fields
            ...
            sublist.setLineItemValue('custpage_schedunsched', s+1, currCase.getText('custevent_release_scheduled') );
            sublist.setLineItemValue('custpage_casequeue', s+1, currCase.getText('custevent_case_queue') );
...
//            var id = currCase.getId();
            ...
            memoResults = nlapiSearchRecord('phonecall','customsearch_release_dashboard_memos', filter_memo);
...
//form.addButton('custpage_setview','Change View',"viewmanage("+incontrol+");");
...
/*
var blankdash = nlapiResolveURL('SUITELET','customscript_importdashsuitelet','customdeploy_importdashsuitelet',null);
window.location= blankdash
//window.location= url
*/
}
...
else //OK, this is the Post
{
    ...
    params.searchday = request.getParameter('custpage_datefilter');
    ...
    var headerRow = "Deal,Case,Subject,Assigned,CompExp,Last Note,Expire,To, Amount, Imp Note,EA,PA,EE,EXP$,SCH,PM,VER,ALL,QL,EL,IL,SL,LA,ESN\n";
    ...
    var caseResults = null;
    var assigedtofilter = request.getParameter('assigedtofilter');  //Check to see if we used a filter - if so apply
    var searchday = request.getParameter('custpage_datefilter');  //Check to see if we used a filter - if so apply
    ...
    if (assigedtofilter)
    {
        ...
        {
            var filter_date = new Array();
            filter_date[0] = new nlobjSearchFilter('datecreated', null, 'onorafter', searchday);
            caseResults = nlapiSearchRecord('supportcase', 'customsearch1039', filter_case);
        }
        else
        {
    ...
    for (var z = 0; caseResults != null && z < caseResults.length; z++ )
    {
        var currCase = caseResults[z];
        var deal = currCase.getValue('companyname','custevent1');
        var casenm = currCase.getValue('casenumber');
        var subject = currCase.getValue('title').replace(/,/g, '');
        var assigned = currCase.getText('assigned');
        var assignedComp = currCase.getText('custevent_assignedto_comp_exp');
        var expire = currCase.getValue('custevent_release_expire_date') ;
        var to = currCase.getText('custevent_case_category').replace(/,/g, ''); 
        var amt = currCase.getValue('custeventreleaseamount');
        var eapa1 =     currCase.getText('custeventreleasecontact'); 
        var eapa2 =    currCase.getText('custeventreleasecontactpa');
        var ee1 = currCase.getText('custeventreleaseexpensefundsdue'); 
        var exp = currCase.getText('custeventrelease');
        var sch = currCase.getText('custeventreleasescheduled'); 
//        var pm = currCase.getText('custeventreleasepayoutmechanics');
//        var v1 = currCase.getText('custeventreleasemoveverified');
//        var all = currCase.getText('custeventreleaseallocated');
//        var ql = currCase.getText('custeventreleasequietletter'); 
//        var el = currCase.getText('custeventreleaseexpirationletter');
//        var il = currCase.getText('custeventreleaseinstructionletter');
//        var sl1 = currCase.getText('custeventreleaseshareholderletter');
//        var lgl = currCase.getText('custeventreleaselegalapproval');
//        var esn = currCase.getText('custeventreleaseescrowstatementnews');
        ...
        //Build CSV file row
        filteredresults += deal + "," + casenm + "," + subject + "," + assigned + "," + assignedComp + "," + lastNoteDate +  "," + expire + "," + to + "," + amt + "," + impnote +
    ...
    var nameofFile='Open Release Export' + today + '_'+ userName + '.csv';
