...
function getTransFields(transIntId) {
	...
	// Created from fields
	textColumns.push(new nlobjSearchColumn('custbody_aqm_createdfrom'));
	textColumns.push(new nlobjSearchColumn('custbody_acq_boxa_name', 'custbody_aqm_createdfrom'));
	textColumns.push(new nlobjSearchColumn('custbody_acq_boxa_address1', 'custbody_aqm_createdfrom'));
	textColumns.push(new nlobjSearchColumn('custbody_acq_boxa_address2', 'custbody_aqm_createdfrom'));
	textColumns.push(new nlobjSearchColumn('custbody_acq_boxa_city', 'custbody_aqm_createdfrom'));
	textColumns.push(new nlobjSearchColumn('custbody_acq_boxa_state', 'custbody_aqm_createdfrom'));
	textColumns.push(new nlobjSearchColumn('custbody_acq_boxa_postal', 'custbody_aqm_createdfrom'));
	textColumns.push(new nlobjSearchColumn('custbody_acq_boxa_country', 'custbody_aqm_createdfrom'));
	textColumns.push(new nlobjSearchColumn('custbody_acq_boxa_tin', 'custbody_aqm_createdfrom'));
	
	// Created From fields
	textColumns.push(new nlobjSearchColumn('createdfrom'));
	textColumns.push(new nlobjSearchColumn('custbody_acq_boxa_name', 'createdfrom'));
	textColumns.push(new nlobjSearchColumn('custbody_acq_boxa_address1', 'createdfrom'));
	textColumns.push(new nlobjSearchColumn('custbody_acq_boxa_address2', 'createdfrom'));
	textColumns.push(new nlobjSearchColumn('custbody_acq_boxa_city', 'createdfrom'));
	textColumns.push(new nlobjSearchColumn('custbody_acq_boxa_state', 'createdfrom'));
	textColumns.push(new nlobjSearchColumn('custbody_acq_boxa_postal', 'createdfrom'));
	textColumns.push(new nlobjSearchColumn('custbody_acq_boxa_country', 'createdfrom'));
	textColumns.push(new nlobjSearchColumn('custbody_acq_boxa_tin', 'createdfrom'));

	textColumns.push(new nlobjSearchColumn('custcol_acq_certnum'));
	textColumns.push(new nlobjSearchColumn('custcol_acq_acquisition_date'));
	textColumns.push(new nlobjSearchColumn('quantity'));
	textColumns.push(new nlobjSearchColumn('custcol_acq_cost_basis'));
	textColumns.push(new nlobjSearchColumn('amount'));
...
function replaceText(prepEmailRec, jobRec, text, textRes){
	...
	if (textRes && textRes.length != 0){
		...
		var createdFromField = (headerLine.getValue('createdfrom') != '') ? 'createdfrom' : 'custbody_aqm_createdfrom';
		...
		// @recipient_id@
		var recipId = '';
		var fullRecipId = headerLine.getValue('custbody_acq_boxa_tin', createdFromField);
		...
		// @recipient_address@
		var recAddr = '';
		if (headerLine.getValue('custbody_acq_boxa_name', createdFromField) != '') 
			recAddr += headerLine.getValue('custbody_acq_boxa_name', createdFromField) + '<br />';
		if (headerLine.getValue('custbody_acq_boxa_address1', createdFromField) != '') 
			recAddr += headerLine.getValue('custbody_acq_boxa_address1', createdFromField) + '<br />';
		if (headerLine.getValue('custbody_acq_boxa_address2', createdFromField) != '') 
			recAddr += headerLine.getValue('custbody_acq_boxa_address2', createdFromField) + '<br />';
		if (headerLine.getValue('custbody_acq_boxa_city', createdFromField) != '') 
			recAddr += headerLine.getValue('custbody_acq_boxa_city', createdFromField) + ', ';
		if (headerLine.getText('custbody_acq_boxa_state', createdFromField) != '') 
			recAddr += headerLine.getText('custbody_acq_boxa_state', createdFromField) + ' ';
		if (headerLine.getValue('custbody_acq_boxa_postal', createdFromField) != '') 
			recAddr += headerLine.getValue('custbody_acq_boxa_postal', createdFromField) + '<br />';
		if (headerLine.getText('custbody_acq_boxa_country', createdFromField) != '') 
			recAddr += headerLine.getText('custbody_acq_boxa_country', createdFromField) + '<br />';
		...
		for (var j = 1; j < textRes.length; j++){
			if (textRes[j].getValue('mainline') != '*') {
				...
				tableData += '<td>';
				if (textRes[j].getValue('custcol_acq_certnum') && textRes[j].getValue('custcol_acq_certnum') != '')
					tableData += textRes[j].getValue('custcol_acq_certnum');
				else 
					tableData += missingText;
				tableData += '</td>';
				
				tableData += '<td>';
				if (textRes[j].getValue('custcol_acq_acquisition_date') && textRes[j].getValue('custcol_acq_acquisition_date') != '')
					tableData += textRes[j].getValue('custcol_acq_acquisition_date');
				else
					tableData += missingText;
				tableData += '</td>';
				
				tableData += '<td>' + addCommas(textRes[j].getValue('quantity').replace('-', '')) + '</td>';
				
				tableData += '<td>';
				if (textRes[j].getValue('custcol_acq_cost_basis') && textRes[j].getValue('custcol_acq_cost_basis') != '')
					tableData += '$' + addCommas(textRes[j].getValue('custcol_acq_cost_basis'));
				else
					tableData += missingText;
				tableData += '</td>';
