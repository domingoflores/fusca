...
function scheduleCopyTemplate(){
    ...
    if (templateProjectTasks != null) {
        for (var i = 0; i < templateProjectTasks.length; i++) {
            ...
            var tempNewTask = createProjectTaskFromTemplate(targetProjectId,templateTask,placeholderResourceId); //just need a very temporary place to put the newly loaded task so we can build the map.
